  ...
  describe("Command API", () => {
    describe("Power commands", () => {
      it("visit w/ no code returns link", async (done) => {
        const res = await request(app).get("/power/off");
      ...
      it("visit w/ correct code", async (done) => {
        ...
        const res2 = await request(app).get(`/power/off?code=${code}`);
      ...
      it("visit w/ incorrect code", async (done) => {
        const res = await request(app).get("/power/off?code=x123");
      ...
      describe("quick check for each", () => {
        it("on", async (done) => {
          const res = await request(app).get("/power/on");
          expect(res.body.link).toBeDefined();
          done();
        });
        it("on", async (done) => {
        ...
        it("cold-smoke", async (done) => {
          const res = await request(app).get("/power/cold-smoke");
          expect(res.body.link).toBeDefined();
          done();
        });
      });
    ...
    describe("Temp commands", () => {
      ...
      it("grill temp 150", async (done) => {
        const res = await request(app).get("/temp/grill?temp=150");
        expect(res.statusCode).toEqual(200);
        const code = res.body.code;
        const link = res.body.link;
        expect(codeGen.newCode).toHaveBeenCalledTimes(1);
        ...
        const expectedLink = url.format({
          protocol: res.request.protocol,
          host: res.request.host,
          pathname: "/temp/grill", // @todo fix res.req.path
          query: {
            temp: 150,
            code,
          },
        ...
        expect(link).toEqual(expectedLink);
        const res2 = await request(app).get(`/temp/grill?temp=150&code=${code}`);
        expect(res2.statusCode).toEqual(200);
      ...
      describe("quick check for each", () => {
        ...
        it("probe1", async (done) => {
          const res = await request(app).get("/temp/probe1");
          expect(res.body.link).toBeDefined();
          done();
        });
        it("probe2", async (done) => {
          const res = await request(app).get("/temp/probe2");
          expect(res.body.link).toBeDefined();
          done();
        });
      });
    ...
    describe("Settings commands", () => {
      it("set pizza mode", async (done) => {
        const res = await request(app).get("/settings/pizza");
        expect(res.statusCode).toEqual(200);
        const code = res.body.code;
        const link = res.body.link;
        expect(codeGen.newCode).toHaveBeenCalledTimes(1);
        ...
        const expectedLink = url.format({
          protocol: res.request.protocol,
          host: res.request.host,
          pathname: "/settings/pizza", // @todo fix res.req.path
          query: {
            code,
          },
        });
        expect(link).toEqual(expectedLink);
        const res2 = await request(app).get(`/settings/pizza?code=${code}`);
        expect(res2.statusCode).toEqual(200);
      ...
      describe("quick check for each", () => {
        ...
        it("regular", async (done) => {
          const res = await request(app).get("/settings/regular");
          expect(res.body.link).toBeDefined();
          done();
        });
      });
